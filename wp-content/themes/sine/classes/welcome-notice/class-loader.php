...
if( !class_exists( 'Sine_Welcome_Notice' ) ){	
	class Sine_Welcome_Notice{
		...
		public static function notice_init( $theme_name = false ){
			...
			# welcome Note
			# welcome Note
			if( 'dismiss_notice' !== get_transient( 'rt_welcome_notice' ) ){
		...
		public static function rt_welcome_ajax_action_close(){
			$response = array(
				'data' => array(
					'message' => esc_html__( 'Invalid Access', 'sine' )
				),
				'status' => 400
			);

			if( wp_verify_nonce( $_POST['nonce'], 'rt-welcome-nonce' ) ){
				set_transient( 'rt_welcome_notice', 'dismiss_notice', 3*MONTH_IN_SECONDS );

				$response[ 'data' ][ 'message' ] = esc_html__( 'Notice dismissed.', 'sine' );
				$response[ 'status' ] = 200;
			}
		...
		public static function rt_welcome_ajax_action(){
			...
			if( wp_verify_nonce( $_POST['nonce'], 'rt-welcome-nonce' ) ){
				...
				$response[ 'data' ][ 'message' ] = esc_html__( 'Plugin Installed and Activated Successfully', 'sine' );
				$response[ 'status' ] = 200;
			}
		...
		public static function module_admin_scripts(){
			...
			$data = array(
				...
				'confirm_msg' => esc_html__( 'Are you sure?', 'sine' ),
				'install_msg' => esc_html__( 'Installing...', 'sine' ),
				'active_msg' => esc_html__( 'Activating...', 'sine' )
			);
