...
class Better_Messages_Component extends BP_Component
{
    ...
    public function setup_admin_bar( $wp_admin_nav = array() )
    {
        ...
        $wp_admin_nav[] = array(
            ...
            'title'  => $title,
            'href'   => Better_Messages()->functions->get_link( get_current_user_id() )
        );
        ...
        if( Better_Messages()->settings['disableFavoriteMessages'] === '0' ) {
            $href = Better_Messages()->functions->add_hash_arg('favorited', [], Better_Messages()->functions->get_link( get_current_user_id() ));
        ...
        if( Better_Messages()->settings['disableNewThread'] === '0' || current_user_can('manage_options') ) {
            $href = Better_Messages()->functions->add_hash_arg('new-conversation', [], Better_Messages()->functions->get_link( get_current_user_id() ));
