...
if ( !class_exists( 'Better_Messages_Rest_Api' ) ):
    ...
    class Better_Messages_Rest_Api
    {
        ...
        public function __construct()
        {
            ...
            add_action( 'wp_ajax_better_messages_renew_nonce', array( $this, 'rest_nonce' ) );
        }
                ...
                foreach ( $messages as $message_id ){
                    ...
                    if( $message ){
                        ...
                        if( $has_access ){
                            ...
                            $last_update    = (float) Better_Messages()->functions->get_message_meta( $message->id, 'bm_last_update', true );
                            $created_time   = (float) Better_Messages()->functions->get_message_meta( $message->id, 'bm_created_time', true );
                            ...
                            $message->favorited = (Better_Messages()->functions->is_message_starred($message->id, $current_user_id)) ? 1 : 0;
                            $message->message   = Better_Messages()->functions->format_message($message->message, (int) $message->id, 'stack', $current_user_id);

                            $message->lastUpdate = $last_update;
                            $message->createdAt  = $created_time;
                            $message->tmpId      = Better_Messages()->functions->get_message_meta( $message->id, 'bm_tmp_id', true );
        ...
        public function openThreads( WP_REST_Request $request ){
            ...
            if( count( $thread_ids ) > 0 ){
                foreach ( $thread_ids as $thread_id ){
                    Better_Messages()->functions->messages_mark_thread_read( $thread_id, get_current_user_id() );
                }
            }
        ...
        public function checkNew( WP_REST_Request $request ) {
            ...
            if( count( $changedThreads ) > 0 ){
                $thread_ids = array_column($return['threads'], 'id');
            ...
            if( $lastClient > 0 ) {
                ...
                if (count($return['deletedMessages']) > 0) {
                    $message_ids = array_column($return['messages'], 'id');
                ...
                if( count( $changedUsers ) > 0 ) {
                    foreach ($changedUsers as $user_id) {
                        $user_ids = array_column($return['users'], 'id');
        ...
        public function suggest_thread( WP_REST_Request $request ){
            ...
            if( ! $forceNew && count( $recipients ) === 1 ){
                return Better_Messages()->functions->get_pm_thread_id( $recipients[0], get_current_user_id(), false );
            }
        ...
        public function get_pm_thread( WP_REST_Request $request ){
            ...
            if( empty( $uniqueKey ) ){
                return Better_Messages()->functions->get_pm_thread_id( $user_id, get_current_user_id(), $create, $subject );
            } else {
                return Better_Messages()->functions->get_unique_pm_thread_id( $uniqueKey, $user_id, get_current_user_id(), $create, $subject );
            }
        }
        ...
        public function delete_messages( WP_REST_Request $request ){
            ...
            foreach( $messages_ids as $message_id ){
                ...
                $canDelete = $message->sender_id === $user_id;
        ...
        public function delete_thread( WP_REST_Request $request ){
            ...
            if( ! apply_filters( 'bp_better_messages_can_delete_thread', true, $thread_id, get_current_user_id() ) ){
                return new WP_Error(
                    'rest_forbidden',
        ...
        public function send_message( WP_REST_Request $request ){
            ...
            if( ! empty($errors) ) {
                ...
                $messages = $this->get_messages( $thread_id, [ $message_id ] );
        ...
        public function get_user_suggestions(){
            ...
            if( ! is_array( $user_ids ) || count( $user_ids ) === 0 ){
                $total_to_get = 12;
                $friends = Better_Messages()->functions->get_friends_sorted( get_current_user_id(), $total_to_get );
                ...
                if( $get_all_users ) {
                    if ( Better_Messages()->settings['searchAllUsers'] === '1' ) {
                        $other_users = Better_Messages()->functions->get_users_sorted(get_current_user_id(), array_keys($friends), $total_to_get);
        ...
        public function get_messages( $thread_id = null, $message_ids = [], $added_users = [], $count = 50, $apply_filters = true ){
            ...
            foreach( $messages as $key => $message ){
                ...
                if( $apply_filters ) {
                    $messages[$key]->favorited = (Better_Messages()->functions->is_message_starred($message->id, $current_user_id)) ? 1 : 0;
                    //$messages[$key]->onsite    = Better_Messages()->functions->format_message($message->message, (int) $message->id, 'site', $current_user_id);
                    $messages[$key]->message   = Better_Messages()->functions->format_message($message->message, (int) $message->id, 'stack', $current_user_id);
                } else {
                ...
                $messages[ $key ]->tmpId      = Better_Messages()->functions->get_message_meta( $message->id, 'bm_tmp_id', true );
            }
        ...
        public function get_erased_threads( $since ){
            ...
            return array_map( 'intval', $wpdb->get_col( $wpdb->prepare(
                "SELECT threadsmeta.bm_thread_id
            FROM " . bm_get_table('threads') . " threads
        ...
        public function load_more( WP_REST_Request $request ){
            ...
            $thread_id = intval($request->get_param('id'));
            $from      = intval($request->get_param('from'));
            ...
            return $this->get_messages( $thread_id, $from );
        }
        ...
        public function get_thread( WP_REST_Request $request ) {
            ...
            if( Better_Messages()->settings['mechanism'] === 'ajax' ) {
                $return['totalUnread'] = Better_Messages()->functions->get_total_threads_for_user( get_current_user_id(), 'unread' );
            }

            $added_user_ids     = array_column($return['users'], 'id');
            ...
            if( count( $message_ids ) > 0 ){
                ...
                if( count( $missed_messages ) > 0 ){
                    $get_missed_messages = $this->get_messages( $thread_id, $missed_messages, array_column($return['users'], 'id') );
        ...
        public function get_threads( $thread_ids = [], $fetch_messages = true, $fetch_users = true, $personal_data = true, $cache = true, $user_id = false ){
            ...
            if( count($thread_ids) > 0 ) {
                ...
                GROUP BY `threads`.`id`
                ORDER BY `messages`.`date_sent` DESC", $current_user_id);
            ...
            foreach ( $get_threads as $thread ){
                ...
                if( $personal_data ){
                    ...
                    $thread_item['permissions'] = [
                        ...
                        'canAudioCall'         => false,
                        'canMaximize'          => true,
                        'canMinimize'          => Better_Messages()->settings['miniChatsEnable'] === '1',
                        'canReply'             => $can_reply,
                ...
                $thread_item = apply_filters('better_messages_rest_thread_item', $thread_item, $thread_item['id'], $thread_item['type'], $personal_data );
                ...
                if( $fetch_messages ) {
                    ...
                    $added_users = array_merge( $added_users, array_column($users, 'id') );
                }
            }
