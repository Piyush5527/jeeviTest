...
if ( !class_exists( 'Better_Messages_Hooks' ) ):
    ...
    class Better_Messages_Hooks
    {
        ...
        public function disable_start_thread_role_to_role_block(&$args, &$errors){
            ...
            if( count( $restrict_to ) > 0 ) {
                foreach ($recipients as $recipient) {
                    $recipient_userdata = get_userdata($recipient);
        ...
        public function mycred_core_charge_new_thread_created( $thread_id, $bpbm_last_message_id ){
            ...
            $user_charge_rate = Better_Messages()->functions->get_user_mycred_charge_rate( $user_id );
        ...
        public function mycred_core_charge_for_new_thread( &$args, &$errors ){
            ...
            $user_charge_rate = Better_Messages()->functions->get_user_mycred_charge_new_thread_rate( $user_id );
            if( $user_charge_rate === 0 ) return false;
        ...
        public function show_in_bbpress_profile(){
            $messages_total = Better_Messages()->functions->get_total_threads_for_user( get_current_user_id(), 'unread' );
        ...
        public function disable_start_thread_if_thread_exist(&$args, &$errors){
            ...
            foreach($recipients as $recipient){
                ...
                $threads = Better_Messages()->functions->find_existing_threads($from, $to);
        ...
        public function disable_start_thread_for_non_friends(&$args, &$errors){
            ...
            foreach( $recipients as $recipient ){
                if( ! friends_check_friendship( get_current_user_id(), $recipient ) ) {
        ...
        public function catch_fast_thread(){
            ...
            if(
                ...
                if( Better_Messages()->settings['singleThreadMode'] == '1' ) {
                    $threads = Better_Messages()->functions->find_existing_threads(get_current_user_id(), $to->ID);
                    if( count($threads) > 0) {
                ...
                $url = Better_Messages()->functions->get_link( get_current_user_id() ) ;
        ...
        public function redirect_standard_component()
        {
            if ( bp_is_messages_component() ) {
                ...
                if( bp_is_current_action('compose') ){
                    $link = Better_Messages()->functions->add_hash_arg('new-conversation', [], Better_Messages()->functions->get_link( get_current_user_id() ));
                }
