    ...
    class Better_Messages_User_Config
    {
        ...
        public function get_user_settings(){
            ...
            if( Better_Messages()->settings['allowUsersRestictNewThreads'] === '1' ) {
                $options = $this->get_who_can_start_options();
                $current = $this->get_who_can_start_value( get_current_user_id() );
            ...
            $notifications =  [
                ...
                'options' => [
                    ...
                    [
                        ...
                        'value' => 'yes',
                        'checked' => (get_user_meta( $user_id, 'bpbm_disable_sound_notification', true ) !== 'no'),
                        'desc'  => _x('When enabled, you will not hear a sound when a new message is received.', 'User settings', 'bp-better-messages')
                    ],
                ]
            ];

            if( Better_Messages()->settings['enablePushNotifications'] === '1' ){
        ...
        public function user_settings_save( WP_REST_Request $request ){
            ...
            switch( $option ){
                ...
                case 'online_status':
                    ...
                    update_user_meta( $user_id, 'bm_last_update', Better_Messages()->functions->get_microtime() );
