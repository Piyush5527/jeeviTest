...
if( ! class_exists( 'BM_Messages_Message' ) ):
    ...
    class BM_Messages_Message {
        ...
        public function send() {
            ...
            if ( ! $wpdb->query( $wpdb->prepare( "INSERT INTO " . bm_get_table('messages') . " ( thread_id, sender_id, message, date_sent ) VALUES ( %d, %d, %s, %s )", $this->thread_id, $this->sender_id, $this->message, $this->date_sent ) ) ) {
                return false;
            }
            ...
            if ( $new_thread ) {
                ...
                foreach ( (array) $this->recipients as $recipient ) {
                    ...
                    VALUES ( %d, %d, 1, %d )", $recipient->user_id, $this->thread_id, $time ) );
        ...
        public static function get_last_sent_for_user( $thread_id ) {
            ...
            $query = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM " . bm_get_table('messages') . " WHERE sender_id = %d AND thread_id = %d ORDER BY date_sent DESC LIMIT 1", bp_loggedin_user_id(), $thread_id ) );
