    ...
    class Better_Messages_Functions
    {
        ...
        public function create_conversation_link( int $user_id, string $subject = '', string $message = '', bool $fast_start = false, bool $scrollToContainer = true ): string
        {
            ...
            if( $fast_start ){
                ...
                return add_query_arg($args, Better_Messages()->functions->get_link( get_current_user_id() ));
            } else {
                return Better_Messages()->functions->add_hash_arg('new-conversation', $args, Better_Messages()->functions->get_link( get_current_user_id() ));
            }
        }
         ...
         * @return array
         */
        public function can_start_conversation($starter_id, array $recipients = [] ): array
        {
            $args = array(
        ...
        public function delete_message( $message_id, $thread_id = false ){
            ...
            if( $thread_id !== false ) {
                ...
                unset( $recipients[get_current_user_id()] );
            ...
            $this->update_message_update_time( $message_id );
        ...
        public function can_add_users_to_thread($user_id, $thread_id){
            ...
            SELECT COUNT(*) FROM `" . bm_get_table('recipients') . "` WHERE `thread_id` = %d AND `is_deleted` = '0'
            ", $thread_id ));

            if( ( $participants_count > 2 && $allow_invite ) || ( $participants_count <= 2 && Better_Messages()->settings['privateThreadInvite'] === '1' ) ){
                $userIsParticipant = (bool) $wpdb->get_var($wpdb->prepare("
                SELECT COUNT(*) FROM `" . bm_get_table('recipients') . "` WHERE `user_id` = %d AND `thread_id` = %d AND `is_deleted` = '0'
        ...
        public function get_user_link( $user_id, $avatar_size = 20 ){
            ...
            if( $url !== false ){
                $link = '<a href="' . bp_core_get_userlink( $user_id, false, true ) . '" class="user bm-user">' . Better_Messages_Functions()->get_avatar( $user_id, $avatar_size ) . $display_name . '</a>';
            } else {
                $link = '<span class="user bm-user">' . Better_Messages_Functions()->get_avatar( $user_id, $avatar_size ) . $display_name . '</span>';
            }
        ...
        public function render_thread( $thread, $user_id = false )
        {
            ...
            if( $thread_type === 'group' ) {
                if( class_exists('BP_Groups_Group') ) {
                    $group_id = Better_Messages()->functions->get_thread_meta($thread->thread_id, 'group_id');
                    $group = new BP_Groups_Group((int)$group_id);
                }

                if( class_exists('PeepSoGroup') ){
                    $group_id = Better_Messages()->functions->get_thread_meta($thread->thread_id, 'peepso_group_id');
                    $group = new PeepSoGroup( (int) $group_id );
                }

                if( class_exists('UM_Groups') ){
                    $group_id = Better_Messages()->functions->get_thread_meta($thread->thread_id, 'um_group_id');
                    $group    = get_post( (int) $group_id );
                }
            }

            if( $thread_type === 'chat-room' ) {
                $chat_id = (int) Better_Messages()->functions->get_thread_meta($thread->thread_id, 'chat_id');
            } else {
                $chat_id = false;
            }
            ...
            $classes = [];
            if ( $thread->unread_count > 0 && Better_Messages()->settings['mechanism'] === 'ajax' ) {
                $classes[] = 'unread';
            }

            $show_avatars = Better_Messages()->functions->show_avatars();
            ...
            ?><div class="thread <?php echo implode(' ', $classes); ?>"
                 ...
                 data-href="<?php echo add_query_arg( 'thread_id', $thread->thread_id, Better_Messages()->functions->get_link( $user_id ) ); ?>">
                <?php if( $show_avatars ) {  ?>
                    ...
                    if( !! $group_id ){
                        ...
                        foreach ( $thread->recipients as $recipient ) {
                            ...
                            if ($userdata) {
                                ...
                                $avatar = Better_Messages_Functions()->get_avatar($recipient, 25);
                                echo '<a href="' . $link . '">' . $avatar . '</a>';
                            } else {
                                $avatar = Better_Messages_Functions()->get_avatar(0, 25);
                                echo $avatar;
                            }

                            if ( $i == 4 ) break;
                        }
                        if ( $i < 4 ) echo Better_Messages_Functions()->get_avatar( $user_id, 25 );
                    } else {
                        if( $recipients_count > 0 ) {
                            ...
                            if ($userdata) {
                                ...
                                $avatar = Better_Messages_Functions()->get_avatar($user_id, 50);
                                echo '<a href="' . $link . '">' . $avatar . '</a>';
                            } else {
                                $avatar = Better_Messages_Functions()->get_avatar(0, 50);
                                echo $avatar;
                            }
                        }
                    } ?>
                ...
                <div class="info">
                    ...
                    } else if( Better_Messages()->settings['disableSubject'] !== '1' ) {
                    ...
                    <p><?php
                        ...
                        echo Better_Messages_Functions()->get_avatar( $thread->user_id, 20 );
                        echo $thread->message;
                        ?>
                    </p>
                </div>
                <div class="time">
                    ...
                    <span class="unread-count"><?php if ( $thread->unread_count > 0 && Better_Messages()->settings['mechanism'] === 'ajax' ) echo '+' . $thread->unread_count; ?></span>
                    </div>
                </div>
        ...
        public function get_private_conversation_id(int $to, int $from = null, bool $create = true, string $subject = '', $uniqueKey = null): array{
            ...
            if(! $from ) $from = get_current_user_id();
            ...
            $can_start = $this->can_start_conversation( $from_user->ID, [ $to_user->ID ] );

            if( $can_start['result'] === 'new_thread' && $create ){
                if( ! $uniqueKey ){
                    $new_thread_id = $this->create_new_conversation( [ $from_user->ID, $to_user->ID ], $subject );
                } else {
                    $new_thread_id = $this->get_unique_conversation_id( [ $from_user->ID, $to_user->ID ], $uniqueKey, $subject );
                }
        ...
        public function get_conversation_layout(int $thread_id ){
            ...
            return '<div class="bp-messages-single-thread-wrap" style="height: ' . $initialHeight . 'px" data-thread-id="' . $thread_id . '">' . Better_Messages()->functions->container_placeholder() . '</div></div>';
        }
        ...
        public function get_thread_type( int $thread_id ): string {
            ...
            if( $thread->type === 'group' ) {
                ...
                } else if (Better_Messages()->settings['PSenableGroups'] === '1') {
                    $group_id = Better_Messages()->functions->get_thread_meta($thread_id, 'peepso_group_id');
        ...
        public function rest_user_item( $user_id, $include_personal = true ){
            ...
            if( $include_personal ){
                $item['isFriend'] = (int) $this->is_friends( get_current_user_id(), $user_id );
        ...
        public function render_me(){
            ...
            if( Better_Messages()->settings['myProfileButton'] !== '1' ) {
                $profile_url = false;
            } else {
            ...
            <span class="bpbm-user-me-avatar"><?php echo Better_Messages()->functions->get_avatar( $user_id, 30 ); ?></span>
            <span class="bpbm-user-info">
                <span class="bpbm-user-me-name"><?php esc_attr_e( Better_Messages()->functions->get_name( $user_id ) ); ?></span>
                <?php if ( $statuses_enabled ){ ?>
                <span class="bpbm-status">
                    <span class="current-status"><?php echo $statuses[$status]['icon']; echo Better_Messages()->websocket->get_status_display_name($status) ;?></span>
                </span>
                <?php } ?>
            </span>
        ...
        public function new_message( $args = '' ) {
            remove_action( 'messages_message_after_save', 'bp_messages_clear_cache_on_message_save' );
            remove_action( 'messages_message_sent', 'bp_messages_clear_cache_on_message_save', 1, 2 );
        ...
        public function update_message_update_time( $message_id, $new_message = false ){
        ...
        public function get_last_activity( $user_id ){
            $last_activity = get_user_meta( $user_id, 'bpbm_last_activity', true  );
        ...
        public function is_thread_moderator( $thread_id, $user_id ){
            $type = Better_Messages()->functions->get_thread_type( $thread_id );
            if( $type !== 'chat-room' ) {
                return false;
            }
        ...
        public function get_moderators( $thread_id ){
            ...
            return (array) array_values($moderators);
        }
        ...
        public function get_message_ranges( $message_ids = [] ){
            ...
            foreach( $message_ids as $message_id ){
                ...
                if( $current_range === false ){
                    $current_range = [ $message_id, $message_id ];
                    continue;
                }

                if( is_array($current_range) ){
                    $to   = $current_range[1];

                    $current_range[1] = $message_id;
                    if( ( $to + 1 ) !== $message_id ){
        ...
        public function get_missed_message_ids( $thread_id, $message_ids = [] ){
            ...
            return $wpdb->get_col($wpdb->prepare("SELECT id
